...
void PlayerShip::SetCamera()
{
	...
	camera->SetPosition(transform->GetPosition() + (transform->GetForward() * CameraDistance));
	camera->SetNewOrientation(FSMath::LookAt(camera->GetPosition() - transform->GetPosition(), FSMath::Up));
}
